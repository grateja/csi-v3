<template>
    <div>
        <v-btn @click="createEvent" class="rounded-card primary">
            ...
            <v-flex sm6 lg4 v-for="event in events" :key="event.id">
                <v-card @click="view(event)" class="event ma-1">
                    ...
                    <v-card-actions>
                        ...
                        <v-btn icon @click="deleteEvent($event, event)" :loading="event.isDeleting">
                            <v-icon>delete_outline</v-icon>
                        </v-btn>
                        <v-spacer></v-spacer>
                        <v-tooltip top v-if="event.is_default">
                        ...
                        <v-tooltip top v-else>
                            <v-btn slot="activator" icon @click="setDefault($event, event)" :loading="event.settingDefault">
...
export default {
    ...
    methods: {
        ...
        view(event) {
            this.$router.push(`/external-view/events/${event.id}`);
        },
        createEvent() {
        ...
        setDefault($e, event) {
            $e.stopPropagation();
            if(confirm(`Set "${event.title}" as default event?`)) {
                Vue.set(event, 'settingDefault', true);
                this.$store.dispatch('event/setDefault', event.id).then((res, rej) => {
                    this.events = this.events.filter(e => e.id != res.data.eventId);
                    this.events.find(e => e.is_default).is_default = false;
                    event.is_default = true;
                    Vue.set(event, 'settingDefault', false);
                }).finally(() => {
                    Vue.set(event, 'settingDefault', false);
                });
            }
        ...
        deleteEvent($e, event) {
            ...
            if(confirm('Are you sure you want to delete this event?')) {
                Vue.set(event, 'isDeleting', true);
                this.$store.dispatch('event/deleteEvent', event.id).then((res, rej) => {
                    this.events = this.events.filter(e => e.id != res.data.eventId);
                    Vue.set(event, 'isDeleting', false);
                }).finally(() => {
                    Vue.set(event, 'isDeleting', false);
                });
            }
