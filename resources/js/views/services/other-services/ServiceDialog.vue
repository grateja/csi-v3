...
export default {
    ...
    methods: {
        close() {
            ...
            this.$store.commit('otherservice/clearErrors');
        },
        submit() {
            this.$store.dispatch(`otherservice/${this.mode}Service`, {
        ...
        savePicture(formData) {
            this.$store.dispatch('otherservice/setPicture', {
        ...
        removePicture() {
            if(confirm('Delete this picture?')) {
                this.$store.dispatch('otherservice/removePicture', this.service.id).then((res, rej) => {
    ...
    computed: {
        errors() {
            return this.$store.getters['otherservice/getErrors'];
        },
        saving() {
            return this.$store.getters['otherservice/isSaving'];
        }
    },
    watch: {
        service(val) {
