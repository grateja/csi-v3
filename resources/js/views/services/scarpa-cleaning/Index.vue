<template>
    <div>
        <v-btn class="ml-0 primary" @click="addService" round v-if="isOwner">
            <v-icon left>add</v-icon> add category
        </v-btn>
...
export default {
    ...
    methods: {
        ...
        load() {
            this.loading = true;
            axios.get('/api/services/scarpa-cleanings').then((res, rej) => {
        ...
        deleteService(item) {
            if(confirm('Do you really want to delete this service?')) {
                ...
                this.$store.dispatch('scarpacleaning/deleteService', item.id).then((res, rej) => {
