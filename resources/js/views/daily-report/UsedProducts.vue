<template>
    <v-card v-if="Object.keys(items).length" class="translucent rounded-card mt-2">
        <v-card-title>
            <h4 class="font-weight-bold title">USED PRODUCTS</h4>
            <v-divider></v-divider>
        ...
        <v-card-text>
            <div v-for="(list, key) in items" :key="key">
                <v-expand-transition>
                    <div v-if="simplified">
                        {{ key }} - {{ total(list).count }} {{ 'pc' | pluralize(total(list).count) }}
                    </div>
                </v-expand-transition>
                <v-expand-transition>
                    <div v-if="!simplified">
                        ...
                        <div v-for="item in list" :key="item.transaction.created_at" @click="$emit('openJobOrder', item.transaction.id)">
                            <v-layout>
                                ...
                                <v-flex xs1>{{item.quantity}} {{ 'pc' | pluralize(item.quantity) }}</v-flex>
                        ...
                        <v-layout class="font-weight-bold" v-if="total(list)">
                            ...
                            <v-flex xs3>{{total(list).count}} {{ 'pc' | pluralize(total(list).count) }}</v-flex>
