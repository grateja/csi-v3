...
export default {
    props: [
        'value', 'serviceName', 'transactionId'
    ],
    data() {
        return {
            loading: false,
    ...
    methods: {
        ...
        load() {
            this.loading = true;
            axios.get(`/api/transactions/${this.transactionId}/view-service-items`, {
                params: {
        ...
        deleteItem(item) {
            if(confirm('Are you sure you want to remove this item?')) {
                ...
                this.$store.dispatch('transaction/deleteServiceItem', item.service_transaction_item_id).then((res, rej) => {
                    this.items = this.items.filter(i => i.service_transaction_item_id != res.data.serviceTransactionItem.id);
                }).finally(() => {
