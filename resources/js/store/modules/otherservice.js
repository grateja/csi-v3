...
const actions = {
    ...
    updateService(context, data) {
        context.commit('setSavingStatus', true);
        context.commit('clearErrors');
        return axios.post(`/api/services/other-services/${data.serviceId}/update`, data.formData).then((res, rej) => {
            context.commit('setSavingStatus', false);
            return res;
        }).catch(err => {
            context.commit('setErrors', err.response.data.errors);
    ...
    setPicture(context, data) {
        ...
        context.commit('clearErrors');
        console.log(data)
        return axios.post(`/api/services/other-services/${data.serviceId}/set-picture`,
            data.formData, {
                headers: {
    ...
    removePicture(context, serviceId) {
        context.commit('setSavingStatus', true);
        context.commit('clearErrors');
        return axios.post(`/api/services/other-services/${serviceId}/remove-picture`).then((res, rej) => {
            context.commit('setSavingStatus', false);
            return res;
        }).catch(err => {
            context.commit('setErrors', err.response.data.errors);
    ...
    deleteService(context, serviceId) {
        return axios.post(`/api/services/other-services/${serviceId}/delete-service`).then((res, rej) => {
            return res;
        }).catch(err => {
            return Promise.reject(err);
        });
